...
/* ------------------------------------------------------------
 *  Funciones de bajo nivel (control directo del bus) - Maestro
 * ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
 *  Modo Esclavo por POLLING (bloqueante, solo receptor)
 * ------------------------------------------------------------ */
...
/* ------------------------------------------------------------
 *  Modo Esclavo por INTERRUPCIÓN (no bloqueante, receptor + transmisor)
 *  Protocolo estilo "mapa de registros" (igual que EEPROM/sensores I2C):
 *
 *  Master escribe:  START, SLA+W, [indice_registro], [dato0, dato1...], STOP
 *  Master lee:      START, SLA+W, [indice_registro], REP-START, SLA+R,
 *                    [dato0, dato1...], STOP
 *
 *  El índice se auto-incrementa en cada byte leído/escrito.
 *  Compatible con i2c_write_reg / i2c_read_reg / i2c_read_multi /
 *  i2c_write_multi usados desde el Maestro.
 * ------------------------------------------------------------ */
/* Configura el esclavo en modo interrupción sobre un mapa de registros propio.
 * direccion       : dirección de 7 bits propia
 * mapa_registros  : arreglo (volatile) que el ISR leerá/escribirá
 * num_registros   : cantidad de registros del arreglo                        */
